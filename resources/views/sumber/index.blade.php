        ...
        <div class="flex-1 max-w-[1600px] mx-auto w-full flex flex-col space-y-4 mt-4">
            ...
            <div class="bg-white px-6 py-4 rounded-2xl border shadow-sm">
                <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
                    ...
                    <form action="{{ route('sources.index') }}" method="GET" class="flex flex-wrap items-center gap-2">
                        ...
                        <select name="category" onchange="this.form.submit()"
                            ...
                            @foreach ($allCategories as $cat)
                                <option value="{{ $cat->id }}" {{-- Pastikan nama di request sama dengan atribut name pada
                                    select --}} {{ request('category') == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <button type="submit"
                            class="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-xs font-semibold hover:bg-blue-700 transition">
                            Filter
                        </button>
