    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
          <h3 class="font-black text-slate-800 uppercase italic tracking-tight text-lg">Data Statistik</h3>

          <div class="flex items-center space-x-2">
            <form action="{{ route('statistics.index') }}" method="GET" class="flex items-center space-x-2">
              <select name="opd_id"
                class="rounded-xl border-gray-200 text-[10px] font-bold uppercase focus:ring-blue-500 py-1 px-3">
                <option value="">OPD</option>
                @foreach($allOpd as $opd)
                  {{-- Ambil nama_opd dari relasi perangkatDaerah milik user --}}
              ...
              <select name="kategori"
                ...
                @foreach(\App\Models\Category::all() as $cat)
                  <option value="{{ $cat->nama_kategori }}" {{ request('kategori') == $cat->nama_kategori ? 'selected' : '' }}>
                    {{ $cat->nama_kategori }}
                  </option>
                @endforeach
              </select>
              ...
              <button type="submit"
                class="bg-slate-800 text-white px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase hover:bg-black transition">Filter</button>

              @if(request()->anyFilled(['opd_id', 'tahun', 'kategori']))
                <a href="{{ route('statistics.index') }}"
                  class="text-rose-500 text-[10px] font-bold uppercase px-2 hover:underline">Reset</a>
              @endif
            </form>
          ...
          <tbody>
            ...
            @forelse($groupedStats as $userId => $stats)
              ...
              <tr class="bg-gray-100/80 border-b border-gray-200">
                <td colspan="4" class="px-6 py-2.5 text-[11px] font-black text-slate-800 uppercase tracking-tight">
              ...
              @foreach($stats as $stat)
                <tr class="hover:bg-slate-50 border-b border-gray-50 group">
                  ...
                  <td class="px-6 py-4 border-r text-slate-700 font-medium text-[11px]">
                    {{ $stat->category ?? 'Sektoral' }}
                  </td>
                  <td class="px-6 py-4 border-r">
                    <span class="text-slate-700 font-medium uppercase text-[11px] leading-relaxed">
                    ...
                    <div class="flex justify-center items-center space-x-2">
                      @if($stat->statisticData->count() > 0)
                        <button type="button" @click="
                                  ...
                                  rows = [{}]; 
                                  openView = true;
                              " class="text-emerald-600 hover:bg-emerald-50 p-1.5 rounded-lg transition" title="Lihat">
                      ...
                      @else
                        @if(auth()->user()->role == 'operator')
                          <button type="button" @click="
                                      ...
                                      rows = [{}]; 
                                      openInput = true;
                                  " class="text-blue-600 text-[10px] font-black uppercase hover:underline">Isi Data</button>
                        @endif
                      @endif
                    </div>
                  </td>
        ...
        <div class="p-6 overflow-y-auto flex-grow space-y-6 bg-slate-50/50">
          ...
          @if(auth()->user()->role == 'operator')
            <div class="bg-blue-50/50 p-6 rounded-3xl border-2 border-dashed border-blue-200">
              <h4 class="text-[10px] font-black text-blue-600 uppercase mb-4 tracking-widest">+ Lanjutkan Pengisian Data
              </h4>
              <form action="{{ route('statistics.store') }}" method="POST">
                ...
                <div class="overflow-x-auto bg-white rounded-2xl border border-blue-100 mb-4 shadow-sm">
                  <table class="w-full text-left text-xs border-collapse">
                    <thead class="bg-blue-600 text-white font-black uppercase">
                      <tr>
                        <th class="px-4 py-3 text-center w-12 border-r border-white/20 italic">BARU</th>
                        <template x-for="col in columns" :key="col">
                          <th class="px-4 py-3 border-r border-white/20" x-text="col"></th>
                        </template>
                        <th class="w-10"></th>
                      </tr>
                    </thead>
                        ...
                        <tr class="border-b last:border-0">
                          ...
                          <template x-for="col in columns" :key="col">
                            <td class="px-4 py-3 border-r border-slate-50">
                              <input type="text" :name="'data_content['+nIndex+']['+col+']'"
                                class="w-full border-transparent focus:ring-0 text-[11px] p-1.5" placeholder="...">
                            </td>
                          </template>
                ...
                <tbody>
                  <template x-for="(row, index) in rows" :key="index">
                    <tr>
                      ...
                      <template x-for="col in columns" :key="col">
                        <td class="px-4 py-3 border-b border-l border-slate-100"><input type="text"
                            :name="'data_content['+index+']['+col+']'"
                            class="w-full border-transparent focus:ring-0 text-[11px] p-1"></td>
                      </template>
                    </tr>
                  </template>
                </tbody>
