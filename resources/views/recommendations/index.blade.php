<x-app-layout>
    <div class="py-6" x-data="{ openAdd: false }">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 space-y-4"> {{-- HEADER MENU --}} <div
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 border-b border-gray-100 font-bold uppercase text-[10px] text-gray-400">
                        <tr>
                            ...
                            @if (auth()->user()->role == 'admin')
                                ...
                                @endif <th class="px-6 py-4">Jadwal Pengisian</th>
                    ...
                    <tbody class="divide-y divide-gray-50 text-sm">
                        @foreach ($recommendations as $rec)
                            <tr x-data="{ openDetail: false, openApprove: false, openCorrect: false, openReject: false, openEdit: false }" class="hover:bg-slate-50/50 transition-colors">
                                {{-- TAMPILAN KODE --}} <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span
                                            class="text-slate-300 italic text-[9px] uppercase tracking-tighter">Pending</span>
                                        @endif
                                </td>
                                <td class="px-6 py-4"> <span
                                        class="bg-blue-50 text-blue-600 text-[9px] font-black px-2 py-0.5 rounded uppercase mb-1 inline-block italic">
                                        {{ $rec->user->perangkatDaerah->nama_opd ?? $rec->user->name }} </span>
                                    <p class="font-bold text-slate-700 leading-tight uppercase">{{ $rec->table_name }}
                                    </p> <button @click="openDetail = true"
                                    ...
                                    @if ($rec->admin_note && ($rec->status == 'corrected' || $rec->status == 'rejected'))
                                        <div
                                            class="mt-3 p-3 bg-amber-50 border-l-4 border-amber-400 rounded-r-xl shadow-sm">
                                ...
                                @if (auth()->user()->role == 'admin')
                                    ...
                                    <td class="px-6 py-4 text-center"> <span
                                            class="px-3 py-1 rounded-full text-[9px] font-black uppercase {{ $rec->status == 'approved' ? 'bg-emerald-100 text-emerald-700' : ($rec->status == 'corrected' ? 'bg-amber-100 text-amber-700' : ($rec->status == 'rejected' ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-500')) }}">
                                            {{ $rec->status }} </span> </td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex flex-col items-center space-y-2">
                                            @if (auth()->user()->role == 'admin' && ($rec->status == 'pending' || $rec->status == 'corrected'))
                                                <div class="flex space-x-2 pt-1 text-[9px] font-black uppercase">
                                                    <button @click="openApprove = true"
                                                        class="text-emerald-600 hover:underline">Setuju</button> <button
                                                        @click="openCorrect = true"
                                                        class="text-amber-600 hover:underline">Koreksi</button> <button
                                                        @click="openReject = true"
                                                        class="text-rose-600 hover:underline">Tolak</button> </div>
                                            ...
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <span>Perbaiki Pengajuan</span> <button
                                                        @click="openEdit = false"><svg
                                                            xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                                                        </svg></button> </div>
                                                <form action="{{ route('recommendations.update', $rec->id) }}"
                                                    method="POST" class="p-6 space-y-4"> @csrf @method('PATCH') <div>
                                                        <label
                                                            class="text-[10px] font-bold text-slate-400 uppercase">Nama
                                                            Tabel</label> <input type="text" name="table_name"
                                                            value="{{ $rec->table_name }}"
                                                            class="w-full rounded-xl border-gray-100 text-sm mt-1"
                                                            required> </div>
                                                    <div> <label
                                                            class="text-[10px] font-bold text-slate-400 uppercase">Susunan
                                                            Kolom</label>
                                                        <textarea name="table_structure" rows="3" class="w-full rounded-xl border-gray-100 text-sm mt-1" required>{{ $rec->table_structure }}</textarea>
                                        ...
                                        </div> {{-- MODAL APPROVE (ADMIN) --}} <div x-show="openApprove"
                                            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4"
                                            x-transition x-cloak>
                                            <div
                                                class="bg-white rounded-3xl w-full max-w-md overflow-hidden text-left shadow-2xl">
                                                <div
                                                    ...
                                                    <span>Setujui & Beri Kode</span> <button
                                                        @click="openApprove = false"><svg
                                                            xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                                                ...
                                                <form
                                                    action="{{ route('recommendations.status', [$rec->id, 'approved']) }}"
                                                    method="POST" class="p-6 space-y-4"> @csrf @method('PATCH') <div>
                                                        <label
                                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Kode
                                                            Tabel Resmi</label> <input type="text" name="table_code"
                                                            placeholder="Contoh: 1.01.01"
                                                            class="w-full rounded-xl border-gray-200 text-xs font-bold mt-1 uppercase"
                                                            required> </div>
                                                    <div> <label
                                                            class="text-[10px] font-black text-slate-400 uppercase">Pilih
                                                            Kategori Statistik</label> <select name="category"
                                                    ...
                                                    <div class="grid grid-cols-2 gap-3">
                                                        <div><label
                                                                class="text-[10px] font-bold text-slate-400 uppercase">Tgl
                                        ...
                                        </div> {{-- MODAL KOREKSI (ADMIN) --}} <div x-show="openCorrect"
                                            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4"
                                            x-transition x-cloak>
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <span>Catatan Koreksi</span> <button
                                                        @click="openCorrect = false"><svg
                                                            xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                                                ...
                                                <form
                                                    action="{{ route('recommendations.status', [$rec->id, 'corrected']) }}"
                                                    method="POST" class="p-6 space-y-4"> @csrf @method('PATCH')
                                                    <div> <label
                                                            class="text-[10px] font-black text-slate-400 uppercase">Pesan
                                                            untuk Operator</label>
                                                        <textarea name="admin_note" rows="4" class="w-full rounded-xl border-gray-200 text-sm mt-1"
                                        ...
                                        </div> {{-- MODAL TOLAK (ADMIN) --}} <div x-show="openReject"
                                            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4"
                                            x-transition x-cloak>
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <span>Tolak Pengajuan</span> <button
                                                        @click="openReject = false"><svg
                                                            xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                                                ...
                                                <form
                                                    action="{{ route('recommendations.status', [$rec->id, 'rejected']) }}"
                                                    method="POST" class="p-6 space-y-4"> @csrf @method('PATCH') <p
                                                        class="text-xs text-slate-500 italic">Mohon berikan alasan
                                                        penolakan permanen.</p>
                                                    <textarea name="admin_note" rows="3" class="w-full rounded-xl border-gray-200 text-sm mt-1"
                                                        placeholder="Alasan penolakan..." required></textarea> <button type="submit"
                                                        class="w-full bg-rose-600 text-white font-black py-4 rounded-xl uppercase text-[10px] shadow-lg">Ya,
                                                        Tolak Permanen</button>
                                                </form>
                                            </div>
                                        </div> {{-- MODAL PREVIEW STRUKTUR --}} <div x-show="openDetail"
                                            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4"
                                            x-transition x-cloak>
                                            <div class="bg-white rounded-2xl w-full max-w-4xl overflow-hidden shadow-2xl text-left"
                                                ...
                                                <div class="p-6 overflow-x-auto">
                                                    <table class="w-full border-collapse border border-slate-200">
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                @foreach (explode(',', $rec->table_structure) as $k)
                                                                    <td
                                                                        class="px-4 py-8 border border-slate-100 shadow-inner">
                                                                    </td>
                                                                @endforeach
                                                            </tr>
                                                        </tbody>
        ...
        </div> {{-- MODAL TAMBAH (OPERATOR) --}} <div x-show="openAdd"
            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4"
            x-transition x-cloak>
            <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl"
                ...
                <div class="p-6 bg-blue-600 text-white font-black uppercase text-sm flex justify-between items-center">
                    <span>Ajukan Struktur Baru</span> <button @click="openAdd = false"
                        class="hover:text-blue-200 transition-colors"> <svg xmlns="http://www.w3.org/2000/svg"
                            class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg> </button> </div>
                <form action="{{ route('recommendations.store') }}" method="POST" class="p-6 space-y-4 text-left">
                    @csrf <div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Nama
                            Tabel</label><input type="text" name="table_name"
                            placeholder="Contoh: Produksi Padi 2024"
                    ...
                    <div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Susunan Kolom
                            (Pemisah Koma)</label>
                        <textarea name="table_structure" rows="3" placeholder="No, Komoditas, Satuan, Nilai"
                            class="w-full rounded-xl border-gray-100 text-sm mt-1 focus:ring-blue-500" required></textarea>
                    </div> <button type="submit"
